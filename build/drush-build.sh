...
function setup_site() {
	if $USE_REF_DB; then
        ...
        echo "Setting up base database"
		$drush si -y --account-name=root_admin --account-pass=7nw7d7@m --site-name="Norman's Glass & Auto Services, Inc." #minimal
	fi
}
...
# If the site is in development, enable the listed development modules.
if $IS_DEVEL; then
	$drush pm-enable $(cat $build_path/mods_enabled_dev | tr '\n' ' ') -y &&
	$drush cache-clear all &&
	$drush updatedb -y &&
	$drush cache-clear all &&
	$drush pm-enable $(cat $build_path/mods_features_enabled_dev | tr '\n' ' ') -y 
	#cache will be cleared after the next features enabled segment
fi
...
$drush features-revert $(cat $build_path/mods_features_enabled | tr '\n' ' ') -y &&
$drush cache-clear all -y
...
# Overwrite vagrant cron with oarsidevcron
# crontab $build_path/oarsidevcron
...
#END SCRIPT
#tput bel will play a terminal noise in some terminal applications
tput bel
echo '********************************************************************************'
echo '********************************BUILD COMPLETE**********************************'
echo '********************************************************************************'
